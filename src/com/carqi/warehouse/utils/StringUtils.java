...
import com.carqi.warehouse.GlobalConstant;
 ...
 *  字符串工具类
 *  @author Yu Liu  DateTime 2013-10-26 上午9:25:44    
 *  @version 1.0
...
public class StringUtils {
	...
	public static Long getDateTrimNullLong(String value){
	  ...
	  *  Function:
	  *	 去除字符串中的空格
	  *  @author Yu Liu  DateTime 2013-11-6 上午11:58:12
	  *  @param str 
	  *  @return 去除空格后的字符串
	  */
	...
	public static String removeExtensions(String num){
	...
	public static String[] getStringData(String cha,String value){
		 List<String> list = new ArrayList<String>();
		if(value.contains(cha)){
			int index = value.indexOf(cha);
			list.add(value.substring(0, index));
			list.add(value.substring(index+1, value.length()));
		}
		return list.toArray(new String[0]);
	}
	
	//去除前后的特殊字符
	public static String dealString (String value,String charat){
		
		if(value.startsWith(charat)){
			value = value.substring(1);
		}
		if(value.endsWith(charat)){
	...
	//提取字符串中的数据
	public static String[] splitString (String value,String character){
		String strTemp = dealString(value, character);
		return strTemp.split("-");
	}
	
	public static String subString (String value,String character){
		int i = value.indexOf(character);
		String strTemp = value.substring(0,i);
		return strTemp;
	}
	
	 //和当前时间大小
    public static boolean dateCompareWithSysDate(String s1){

        Long d1 = DateFormatUtils.parseDateToLong(s1);
        Long d2 = DateFormatUtils.systemDate();
        // 比较
        if (d1 > d2) {
            return true;
        } else {
